...
function api_not_allowed($print_headers = false) {
	...
	if (!empty($_SERVER['REQUEST_URI']) && (!empty($_GET['cidReq']) || $this_section == SECTION_MYPROFILE)) {
		...
		echo '<div id="content">'.get_lang('NotAllowedOrSessionTimeout').'<br />'.get_lang('PleaseLoginAgainFromFormBelow').'<br />'.$test.'</div>';
	...
	echo get_lang('NotAllowedOrSessionTimeout').'<br /><br /><a href="'.$home_url.'" target="_top">'.get_lang('PleaseLoginAgainFromHomepage').'</a><br />';
	echo '</div>';
	if ($print_headers && $origin != 'learnpath') { Display::display_footer(); }
	die();
}
...
<?php
	echo "<form style=\"float:$style;\" id=\"lang_form\" name=\"lang_form\" method=\"post\" action=\"".api_get_self()."\">", "<select name=\"language_list\" onchange=\"javascript: jumpMenu('parent',this,0);\">";
